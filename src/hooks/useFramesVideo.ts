...
import type { ConfiguracaoProvedorFrames } from '../services/iaService'
...
export function useFramesVideo() {
  ...
  const gerarRoteiroFrames = useCallback(
    async (imagens: ImagemAnexada[], config: ConfiguracaoProvedorFrames) => {
      setCarregando(true)
      setErro(null)
      try {
        ...
        const resposta = await iaService.gerarFramesVideo(payload, config)
